...
Here “baseline” means the version of the code that we submitted for **Unit 2**, i.e. *before* any Unit-3 optimizations (trajectory reuse in Part A, parallel Part C, etc.).

We measured wall–clock time on my laptop using `/usr/bin/time -p`:
...
These numbers provide the reference point against which we evaluate the Unit-3 optimizations.

---

## 3. Profiling Part A (posterior panels, n = 1000)

We profiled a single `parta_panels` run using the **Unit-2** implementation:
...
```bash
python -m pstats results/profile_parta_n1000_M4000_N2000.pstats
# inside the prompt:
sort cumtime
stats 20
```

we obtain (top entries only):
...
For Part C we profile a single continuation experiment using the **sequential (1-worker) path**:
...
## 5. Computational complexity – baseline behaviour

### 5.1 Part A runtime vs sample size n

To study how runtime scales with the sample size $n$ in the **baseline (Unit-2) implementation**, we fixed:
...
and ran `src_cli.parta_panels` for a grid of $n$ values using a small helper script:

```bash
python -m scripts.complexity_parta_baseline
# (this script was only used during Unit 2; in Unit 3 we replaced it
#  with the comparison script described in OPTIMIZATION.md.)
```

The script timed a single `parta_panels` call for each $n$ and wrote
`results/complexity_parta_baseline.csv` (columns: `n`, `runtime_sec`).

Measured runtimes from this baseline script:
...
We also checked these values using `/usr/bin/time -p` on individual runs:
...
This behaviour does **not** mean that the algorithm becomes cheaper as $n$ grows. Instead, combined with the profiling results, it shows that:
...
Empirically, over $n ∈ \{100, 300, 600, 1000\}$, the wall-clock runtime is therefore approximately **flat in $n$** for the baseline Part A implementation, because plotting and other non-$n$-dependent work dominate the total cost.

### 5.2 Part C runtime vs continuation length L

For Part C we also examined how the **sequential** runtime scales with the continuation length $L$.  
We fixed:
...
and writes `results/complexity_partc_L.csv` (columns: `L`, `runtime_sec`).

The observed runtimes were:

- $L = 10{,}000$: 12.39 s  
- $L = 30{,}000$: 34.38 s  
- $L = 50{,}000$: 56.34 s  

This is consistent with an approximately **linear** scaling in $L$, as expected from the $\mathcal{O}(M × L)$ cost of the continuation step (`draw_polya_next` is called once for every extra draw).
...
- `make all`
- `make profile-parta`
- the complexity scripts described above,
- and additional `/usr/bin/time -p python -m src_cli.parta_panels ...` runs for selected $n$.
...
Overall, the baseline implementation appears **numerically stable** for the configurations tested in this phase.

---

## 7. Pointer to optimization results

The Unit-3 optimization work builds on this baseline.  
For details of the code changes, speedups, and performance plots (including baseline vs optimized comparisons), see:

- `docs/OPTIMIZATION.md`, and  
- the figures in `results/figures/` (e.g. `perf_parta_runtime_vs_n.*`, `perf_components_baseline_vs_optimized.*`).
